...
def handle_plots(
    ...
    tick_horizon: str,
    tick_earnings_dates: list[str],
    tick_name: str,
    tick_currency: str = "Currency Undefined",
    raw_period: str = "3mo",
    raw_interval: str = "1d",
) -> None:
    """
    ...
    fig.add_trace(
        go.Candlestick(x=[], open=[], high=[], low=[], close=[], name="Prices"),
        row=1,
        col=1,
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=[], y=[], name="Sentiment", line_color=palette["dark"], showlegend=False
        ),
        row=1,
        col=1,
        secondary_y=True,
...
with col_inp_2:
    ticker_input = st.text_input(
        label="Ticker: ", value="GOOG", max_chars=None, label_visibility="collapsed"
    )

with col_inp_3:
    period_dd = st.selectbox(
        label="Period: ",
...
with col_but_2:
    if st.button("Generate plot"):
        ...
        sl_period = (
            "3mo"
            if period_dd == "Last 3 months"
            else "6mo"
            if period_dd == "Last 6 months"
            else "1y"
        )
        ...
        with col_link_2:
            st.link_button(
                "GitHub.com/essteer :arrow_right:", "https://github.com/essteer"
            )
